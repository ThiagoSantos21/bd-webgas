...
SELECT DISTINCT bairro.nome, posto_combustivel.nomeCombustivel
FROM posto_combustivel
JOIN posto ON posto_combustivel.cnpjPosto = posto.cnpj
JOIN bairro ON posto.nomeBairro = bairro.nome
WHERE bairro.nome = 'Cambuí';

-- Mostrar o preço de cada combustível em cada posto
SELECT 
    posto.nome_fantasia,
...
JOIN posto ON preco.cnpjPosto = posto.cnpj
JOIN bairro ON bairro.nome = posto.nomeBairro
WHERE bairro.nome = 'Cambuí'  -- Podemos modificar
...
-- Lista dos bairros que não possuem nenhum posto cadastrado
SELECT bairro.nome
FROM bairro
LEFT JOIN posto ON bairro.nome = posto.nomeBairro
WHERE posto.cnpj IS NULL;

-- Lista de consumidores que possuem mais de um veículo cadastrado
SELECT 
    pessoa.nome,
    COUNT(veiculo.placa) AS quantidade_veiculos
FROM pessoa
JOIN veiculo ON pessoa.login = veiculo.loginPessoa
JOIN usuario ON pessoa.loginUsuario = usuario.login
JOIN tipo_usuario ON usuario.nomeTipoUsuario = tipo_usuario.nome
WHERE tipo_usuario.nome = 'Consumidor'
GROUP BY pessoa.login, pessoa.nome
HAVING COUNT(veiculo.placa) > 1
